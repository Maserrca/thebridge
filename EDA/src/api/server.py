...
def home():
    """ Default path """
    return "Homicidios aquí"

@app.route('/give_me_id', methods=['GET'])
def give_id():
    x = request.args['token_id']
    if x == "M53994161": 
        return return_json(dir(dir(dir(__file__))) + os.sep + "data" + os.sep + "homi_final.csv")
    else:
        return "Wrong password"

def main():
    
    settings_file = os.path.dirname(__file__) + os.sep + "json.json"
